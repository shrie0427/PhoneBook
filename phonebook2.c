/*  Name : Shreeshail Nimankar
    Project Name : Phonebook
    Description : By using this file we can save contacts and do following operations in it
    1) view all contacts
    2) Add a contact
    3) remove a contact
    4) see details of any contact
    5) update details of any contact
    6) delete complete phonebook
*/ 

//Below are the necessary Header files
#include <stdio.h>
#include <stdlib.h>
#include <string.h>


struct person  //use of struct == collection of variables (can be diff types) under a single name
{
    char name[30];
    char country_code[4];
    ...
    while(1)
    {
        ...
        switch(choice)
        {
            case 1:
                list_record();
                getchar();
                getchar();  //two getchar because one for return function of list_record and second for holding terminal
                break;
            case 2:
                add_person();
                getchar();
...
{
    system("cls");
    FILE *fp;  //file pointer to perform file handling operations
    fp = fopen("phonebook_db", "ab+");
    if (fp == NULL)  //if error occurs in opening the file
    {
